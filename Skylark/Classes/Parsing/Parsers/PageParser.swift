...
private extension PageParser {
    ...
    func pageFromJSONFile(url: URL) -> Page? {
        ...
        var elementMapping: [ElementType: [String: String]] = [:]
        let pageName: String = pageDict["name"] as? String ?? fileNameWithoutJSONExt(fileURL: url)
        ElementType.allCases.forEach({ elementType in
    ...
    private func elementMap(pageDict: [String: Any], element: ElementType) -> [String: String] {

...
class FeatureFileParsingService: ParsingService {
    ...
    private func feature(from featureFileText: String) -> Result<T, ParsingError> {
        if let featureDefinition = featureDefinition(from: featureFileText),
            ...
            let feature = Feature(name: name, tags: featureTags, scenarios: scenarios, tagExpression: tagExpression, text: text)
            return .success(feature)
        }
        return .failure(.unexpectedFormat(nil))
    }
    
    func scenarios(from scenariosText: String) -> Result<[Scenario], ParsingError> {
    ...
    private func parsedScenarios(from featureFileContent: String) -> [Scenario]? {
        if let texts = scenarioTexts(from: featureFileContent) {
            let scenarios: [Scenario] = texts.compactMap { scenarioText in
                ...
                return Scenario(name: name, tagExpression: tagExpression, tags: scenarioTags, text: text, type: type)
            }
            return scenarios
        }
        return nil
    ...
    private func scenarioType(from scenarioText: String) -> ScenarioType {
        for type in ScenarioType.allCases {

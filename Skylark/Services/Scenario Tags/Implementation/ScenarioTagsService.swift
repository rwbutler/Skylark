...
struct ScenarioTagsService {
    ...
    private func shouldExecuteScenario(_ scenario: String) -> Bool {
    ...
    /// Evaluates whether the supplied scenario should be executed given the tags.
    func shouldExecuteScenario(tags: [String]) -> Bool {
        return shouldExecuteScenario(tagExpression: tagExpression, scenarioTags: tags)
    }

}

// Private API
internal extension ScenarioTagsService {
    ...
    private func compoundEvaluable(expression: String, tags: [String]) -> Step? {
        ...
        for conjunction in BinaryConjunction.allCases {
            guard let components = conjunction.split(expression),
                let lhs = compoundEvaluable(expression: components.0, tags: tags),
                let rhs = compoundEvaluable(expression: components.1, tags: tags),
                let compoundStep = conjunction.step(lhs: lhs, rhs: rhs) else {
                    continue
            }
            return compoundStep
        }
    ...
    /// If the same conjunction is used more than once in an expression we need to join the rhs components
    /// before processing as a sub-expression.
    private func rhs(from stringComponents: [String], conjunction: String) -> String {
        return stringComponents.joined(separator: conjunction)
    }
    ...
    private func simpleEvaluable(expression: String, tags: [String]) -> Step? {
        if isTag(expression) {
            return SimpleStep({ tags.map { $0.lowercased() }.contains(expression.lowercased()) })
        }
        let prefix = "not "
